...
:root{
  ...
  /* 對齊與版面節奏 */
  --page-x: 22px;          /* Header/Main/Footer 左右一致邊距 */
  --section-gap: 18px;     /* 主要區塊之間的垂直留白 */
  --block-gap: 12px;       /* 區塊內元件的垂直留白 */
}
...
header{
  ...
  color:#fff;
  padding:26px var(--page-x) 14px; /* 左右與主內容對齊 */
  text-align:center;
...
.main-content{
  display:block;
  width:100%;
  padding:0 var(--page-x);  /* 與 header/footer 對齊 */
...
.card-header > *{ min-width:0; } /* 允許內容縮小避免撐爆 */
.country{font-weight:800;color:var(--ink);font-size:16.5px}
.terms{color:var(--accent-700);font-weight:600;font-size:13px;margin-bottom:6px;overflow-wrap:anywhere}
.summary{color:var(--ink-2);font-size:13.5px;line-height:1.75;overflow-wrap:anywhere}
...
.fulltext{margin-top:10px;padding-top:8px;border-top:1px dashed var(--line);color:var(--ink-2);font-size:14px;overflow-x:hidden}
...
.variant-body{font-size:14px;color:var(--ink-2);overflow-wrap:anywhere}
...
.link-row{
  ...
  flex-wrap:wrap;         /* 允許換行 */
  gap:8px;
  overflow:visible;       /* 不要水平卷軸 */
  padding-bottom:0;
  row-gap:10px;           /* 兩行間距 */
}
.link-row .btn{
  flex:1 1 calc(50% - 8px); /* 每行兩顆，平均寬 */
  white-space:nowrap;
  text-align:center;
}
@media (max-width:360px){
  .link-row .btn{ flex:1 1 100%; } /* 超窄螢幕一行一顆，保險 */
}
...
/* =============== Matrix table（eligibility / reassessment / priority / characteristics） =============== */
.matrix{
  background:var(--card);border:1px solid var(--line);border-radius:var(--radius);
...
.ai-msg .bubble{
  ...
  line-height:1.75;font-size:14.5px;color:var(--ink-2);max-width:100%;overflow-wrap:anywhere
}
